...
class Controller extends Lib\Base\Controller
{
    ...
    private function _buildAppointmentsForFC( $staff_id, Lib\Query $query )
    {
        ...
        foreach ( $appointments as $key => $appointment ) {
            ...
            $description['{service_price}']    = Lib\Utils\Common::formatPrice( $appointment['service_price'] );
            ...
            if ( $appointment['number_of_persons'] == $appointment['total_number_of_persons'] ) {
                ...
                if ( $appointment['total'] ) {
                    $description['{total_price}'] = Lib\Utils\Common::formatPrice( $appointment['total'] );
                    $description['{amount_paid}'] = Lib\Utils\Common::formatPrice( $appointment['paid'] );
                    $description['{amount_due}']  = Lib\Utils\Common::formatPrice( $appointment['total'] - $appointment['paid'] );
                    $description['{total_price}'] = Lib\Utils\Common::formatPrice( $appointment['total'] );
