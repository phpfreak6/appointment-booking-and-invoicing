		...
		$.ajax({
            ...
            success  : function(response) {
				...
				response.forEach(function (s, i, o) {
					$("#chatLi").append('<div><strong>Appointment Date:</strong> '+s.appointment_date+' &nbsp; - &nbsp; <strong>Staff Name:</strong> '+s.staff_name+'<br/><strong>Service:</strong>'+s.title+'</div>')
					if(s.text)
						$("#chatLi").append('<br/><strong>Text:</strong>'+s.text+'')
					if(s.note)
						$("#chatLi").append('<br/><strong>Notiz:</strong>'+s.note+'')
					$("#chatLi").append('<hr/></div>')
				});
            }
        });
	});
