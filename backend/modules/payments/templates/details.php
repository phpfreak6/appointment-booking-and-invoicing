            ...
            <tbody>
                <?php foreach ( $items as $item ) :
                    ...
                    <tr>
                        ...
                        <?php $deposit = \Bookly\Lib\Proxy\DepositPayments::prepareAmount( $item['number_of_persons'] * ( $item['service_price'] + $extras_price ), $item['deposit'], $item['number_of_persons'] ) ?>
                        <?php if ( $deposit_enabled ) : ?>
                            <td class="text-right"><?php echo \Bookly\Lib\Proxy\DepositPayments::formatDeposit( $deposit, $item['deposit'] ) ?></td>
                        <?php endif ?>
                        <td class="text-right">
            ...
            <tfoot>
                ...
                <?php if ( $payment['total'] != $payment['paid'] ) : ?>
                    <tr>
                        ...
                        <td class="text-right"><i><?php echo \Bookly\Lib\Utils\Common::formatPrice( $payment['paid'] ) ?></i></td>
                    </tr>
                    <tr>
                        <td colspan="<?php echo 2 + (int) $deposit_enabled ?>"><i><?php _e( 'Due', 'bookly' ) ?></i></td>
                        <td class="text-right"><i><?php echo \Bookly\Lib\Utils\Common::formatPrice( $payment['total'] - $payment['paid'] ) ?></i></td>
                    </tr>
                    <tr>
