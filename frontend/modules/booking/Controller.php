...
class Controller extends Lib\Base\Controller
{
    ...
    public function executeRenderService()
    {
        ...
        if ( $form_id ) {
            ...
            if ( Lib\Config::useClientTimeZone() ) {
                ...
                Lib\Slots\TimePoint::$client_timezone_offset = - $time_zone_offset * MINUTE_IN_SECONDS;
                Lib\Slots\DatePoint::$client_timezone = Lib\Utils\DateTime::guessTimeZone(  Lib\Slots\TimePoint::$client_timezone_offset );
    ...
    public function executeRenderRepeat()
    {
        ...
        if ( $userData->load() ) {
            ...
            if ( $repeat_data ) {
                $until_ts = strtotime( $repeat_data['until'] );
                ...
                $schedule = Lib\Proxy\RecurringAppointments::buildSchedule(
                    clone $userData,
                    Lib\Config::useClientTimeZone()
                        ? Lib\Utils\DateTime::applyTimeZoneOffset( $slots[0][2], $userData->get( 'time_zone_offset' ) )
                        : $slots[0][2],
                    date( 'Y-m-d H:i:s', $until_ts ),
                    $repeat_data['repeat'],
                    $repeat_data['params'],
                        ...
                        switch ( $column ) {
                            ...
                            case 'price':
                                if ( $cart_item->get( 'number_of_persons' ) > 1 ) {
                                    $items_data[ $cart_key ][] = $nop_prefix . Lib\Utils\Common::formatPrice( $cart_item->getServicePrice() - $cart_item->getExtrasAmount() ) . ' = ' . Lib\Utils\Common::formatPrice( ( $cart_item->getServicePrice() - $cart_item->getExtrasAmount() ) * $cart_item->get( 'number_of_persons' ) );
                                } else {
                                    $items_data[ $cart_key ][] = Lib\Utils\Common::formatPrice( $cart_item->getServicePrice() - $cart_item->getExtrasAmount() );
                                }
                                break;
                            case 'deposit':
    ...
    public function executeRenderDetails()
    {
        ...
        if ( $userData->load() ) {
            ...
            $info_text = $this->_prepareInfoText( 6, Lib\Utils\Common::getTranslatedOption( 'bookly_l10n_info_details_step' ), $userData );
            $info_text_guest = ( get_current_user_id() == 0 ) ? $this->_prepareInfoText( 3, Lib\Utils\Common::getTranslatedOption( 'bookly_l10n_info_details_step_guest' ), $userData ) : '';
