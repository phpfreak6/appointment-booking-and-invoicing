        ...
        <?php foreach ( $columns as $column ) :
            switch ( $column ) :
                ...
                case 'time' : ?>
                    ...
                    break;
                case 'price' : ?>
                    <td style="text-align:right!important;"><?php echo \Bookly\Lib\Utils\Common::formatPrice( ( $app['price'] + $extras_total_price ) * $app['number_of_persons'] ) ?></td><?php
                    break;
                case 'status' : ?>
                ...
                case 'cancel' :
                    ...
                    <?php if ( $app['start_date'] > current_time( 'mysql' ) ) : ?>
                        <?php if ( $allow_cancel < strtotime( $app['start_date'] ) ) : ?>
                            <?php if (
                                ( $app['appointment_status'] != \Bookly\Lib\Entities\CustomerAppointment::STATUS_CANCELLED )
                                && ( $app['appointment_status'] != \Bookly\Lib\Entities\CustomerAppointment::STATUS_REJECTED )
                            ) : ?>
