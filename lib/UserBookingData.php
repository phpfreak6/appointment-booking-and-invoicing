...
class UserBookingData
{
    ...
    public function load()
    {
        ...
        if ( $data !== null ) {
            ...
            // Client time zone.
            if ( $data['time_zone_offset'] !== null && Config::useClientTimeZone() ) {
                Slots\TimePoint::$client_timezone_offset = - $data['time_zone_offset'] * MINUTE_IN_SECONDS;
                Slots\DatePoint::$client_timezone = Utils\DateTime::guessTimeZone(  Slots\TimePoint::$client_timezone_offset );
            }
        ...
        foreach ( $data as $field_name => $field_value ) {
            switch ( $field_name ) {
                ...
                case 'email':
                    ...
                    break;
                case 'phone':
                    $validator->validatePhone( $field_name, $field_value, get_option( 'bookly_cst_required_phone' ) );
                    break;
                case 'cart':
    ...
    public function save( $payment_id = null )
    {
		...
		if( !empty($customer->get('id')) ){
			setcookie("bookUID", $customer->get('id'), time() + (86400 * 90), "/"); // 86400 = 1 day
		}
