...
class CustomerAppointment extends Lib\Base\Entity
{
     ...
     * @return array
     */
    public function getCustomFields( $locale = null )
    {
        $service_id = null;
        ...
        if ( $this->get( 'custom_fields' ) != '[]' ) {
            $custom_fields = array();
            foreach ( Lib\Utils\Common::getTranslatedCustomFields( $service_id, $locale ) as $field ) {
    ...
    public function getFormattedCustomFields( $format, $locale = null )
    {
        ...
        switch ( $format ) {
            case 'html':
                foreach ( $this->getCustomFields( $locale ) as $custom_field ) {
