...
class DateTime
{
    ...
    {
        // If offset is 0 then return UTC.
        if ( $offset == 0 ) {
            return 'UTC';
        }

        $is_dst = (int) Lib\Slots\DatePoint::now()->format( 'I' );

        // Attempt to guess the timezone string from the offset.
        if ( $timezone = timezone_name_from_abbr( '', $offset, $is_dst ) ) {
            return $timezone;
        }

        // Fallback to offset.
        return sprintf('+%02d:%02u', $offset / HOUR_IN_SECONDS, $offset / MINUTE_IN_SECONDS % 60 );
    }
}