...
class Notifier
{
    ...
    public function processNotification( Entities\Notification $notification )
    {
        ...
        switch ( $notification->get( 'type' ) ) {
            case 'staff_agenda':
                if ( $this->hours >= $hours[ $notification->get( 'type' ) ] ) {
                    /** @var \stdClass[] $rows */
            ...
            case 'client_reminder_3rd':
                $appointments = $this->getClientReminderAppointments( $notification, $hours[ $notification->get( 'type' ) ] * 60 /* minutes */ );
                if ( $appointments ) {
                    foreach ( $appointments as $ca ) {
                        if ( ! in_array( $ca['compound_token'], $compounds ) ) {
                            $customer_appointment = new Entities\CustomerAppointment( $ca );
     ...
     * @return array
     */
    private function getClientReminderAppointments( Entities\Notification $notification, $minutes_interval )
    {
        /** @var \wpdb $wpdb */
        global $wpdb;
        ...
        return $wpdb->get_results(
            'SELECT `ca`.*
                     FROM `' . Entities\CustomerAppointment::getTableName() . '` `ca`
                LEFT JOIN `' . Entities\Appointment::getTableName() . '` `a` ON `a`.`id` = `ca`.`appointment_id`
                    WHERE `ca`.`status` != "' . Entities\CustomerAppointment::STATUS_CANCELLED . '" 
                      AND `ca`.`status` != "' . Entities\CustomerAppointment::STATUS_REJECTED . '"
                      ...
                      AND NOT EXISTS (
                            SELECT * FROM `' . Entities\SentNotification::getTableName() . '` `sn`
                             WHERE `sn`.`created` >= DATE_SUB("' . $this->mysql_now . '", INTERVAL 14 DAY)
                               AND `sn`.`gateway`  = "' . $notification->get( 'gateway' ) . '"
                               AND `sn`.`type`     = "' . $notification->get( 'type' ) . '"
                               AND `sn`.`ref_id`   = `ca`.`id`
                    ) ORDER BY `a`.`start_date`',
            ARRAY_A
        );
